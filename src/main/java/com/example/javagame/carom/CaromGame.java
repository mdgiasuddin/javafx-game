package com.example.javagame.carom;
 ...
 */
public class CaromGame extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        scene.setOnKeyPressed(e -> {
            if (e.getCode() == KeyCode.R) newGame();
        });
        ...
        primaryStage.setTitle("Carrom - 2 Player");
    ...
    private void buildBoard() {
        ...
        root.getChildren().addAll(outerRing, innerRing, queenSpot);
        ...
        for (double[] c : pocketCenters()) {
            ...
            root.getChildren().addAll(rim, hole);
        }
        ...
        root.getChildren().add(strikerZone);
    ...
    private void buildAimVisuals() {
        ...
        root.getChildren().addAll(dragLine, aimLine, aimTip);
    }
    ...
    private void onMousePressed(MouseEvent e) {
        ...
        boolean onStriker = Math.hypot(e.getX() - striker.x, e.getY() - striker.y) <= striker.radius + 14;
        boolean inBand = Math.abs(e.getY() - baseline) <= STRIKER_RADIUS + 12
                && e.getX() >= strikerMinX() - STRIKER_RADIUS
                && e.getX() <= strikerMaxX() + STRIKER_RADIUS;

        if (inBand && !onStriker) {
            // Tapping the band snaps the striker there; the drag may still become an aim.
            placeStrikerAt(e.getX());
        }
        // Either gesture starts out ambiguous - the first real drag decides which it is.
        dragMode = (onStriker || inBand) ? DragMode.UNDECIDED : DragMode.NONE;
    }

    private void onMouseDragged(MouseEvent e) {
        if (phase != Phase.READY || dragMode == DragMode.NONE) return;

        if (dragMode == DragMode.UNDECIDED) {
            double dx = e.getX() - pressX;
            double dy = e.getY() - pressY;
            if (Math.hypot(dx, dy) < 6) return;
            boolean stayedOnBaseline = Math.abs(e.getY() - baselineY(player1Turn)) <= STRIKER_RADIUS + 10;
        ...
        if (dragMode == DragMode.MOVE) {
            ...
            computeAim(e.getX(), e.getY());
            showAimVisuals();
        }
    }

    private void onMouseReleased(MouseEvent e) {
        // Always clear the gesture and the overlay, even if the shot is not taken.
        DragMode mode = dragMode;
        dragMode = DragMode.NONE;
        hideAimVisuals();

        if (mode != DragMode.AIM || phase != Phase.READY) return;
    ...
    private void updateHud() {
        ...
        if (queenOwner != null) queenLabel.setText("Queen claimed by " + queenOwner.name);
        else if (queenPendingCoverBy != null) queenLabel.setText(queenPendingCoverBy.name + " must cover the Queen");
        else queenLabel.setText("Queen on board");
    }
